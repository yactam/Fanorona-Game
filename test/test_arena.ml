...
let run_game_AI =
  Lwt_main.run (arena (Fanorona.Arena.pair ~w:(player_random W) ~b:(player_random B)))
...
let mult_run_test =
  let open QCheck in 
  let iter = 2 in
  Test.make ~count:iter ~name:(Format.asprintf "%d iterations of AI game run" iter)
  (init_board_arbitrary) (fun b ->
    Format.printf "%a" pp_board b;
    let result = Lwt_main.run (arena (Fanorona.Arena.pair ~w:(player_random W) ~b:(player_random B)) ~init_board:b) in
      ...
      ("pp test", 
        [ 
          ...
          test_case "pp_endgame" `Quick (fun () -> 
            ...
            (match result.endgame with
            | Win x -> Format.asprintf "Player %a has won the game" pp_player x
